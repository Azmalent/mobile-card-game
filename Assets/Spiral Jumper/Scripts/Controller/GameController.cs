...
namespace SpiralJumper.Controller
{
    public class GameController : MonoBehaviour
    {
        ...
        private View.Map m_mapView = null;
        private Model.EndlessMap m_map = null;
        private PlayerController m_player = null;
        private Screens.GameScreenBase m_gameScreen = null;
        ...
        private View.PlatformEffector m_hilightedEffector = null;
        private int m_hilightedSector = -1;


        private void Awake()
        {
            m_mapView = Instantiate(SpiralJumper.get.MapViewPrefab).GetComponent<View.Map>();
            m_mapView.transform.parent = transform;

            var mapParams = SpiralJumper.get.MapParams;
            m_map = new Model.EndlessMap(mapParams.data);
            
            m_mapView.Init(m_map);

            var playerPrefab = SpiralJumper.get.PlayerControllerPrefab;
        ...
        private void UpdatePlatforms()
        {
            ...
            if (platform != null)
            {
                ...
                {
                    if (platform.Type == Model.PlatformType.SaveRing && m_mapView.CurrentLevel + 1 == Level)
                    {
                        Level++;
        ...
        private void InitGameScreen()
        {
            ...
            {
                m_gameScreen = Instantiate(SpiralJumper.get.GameScreenPrefab).GetComponent<Screens.GameScreenBase>();
        ...
        private void OnPlatformEnter(View.PlatformEffector effector, int sector)
        {
            if (m_gameOver)
                return;
        ...
        {
            if (m_mapView.CurrentPlatform.Type != Model.PlatformType.SaveRing)
            {
        ...
        private void GameOver(bool success, View.PlatformEffector effector, int sector = -1)
        {
            m_gameOver = true;
        ...
        public void Continue()
        {
            ...
            if (current != null && current.Type != Model.PlatformType.SaveRing) current.Visible = false;
            if (next != null && next.Type != Model.PlatformType.SaveRing) next.Visible = false;
        ...
        {
            Destroy(gameObject);
            Instantiate(SpiralJumper.get.GameControllerPrefab);
        }
