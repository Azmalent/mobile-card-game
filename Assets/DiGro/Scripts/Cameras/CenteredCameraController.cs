...
public class CenteredCameraController : CenteredCamera {
    ...
    private void OnSwipe(Vector2 delta, KeyCode code) {
        if (code == KeyCode.Mouse2) {
            ...
            if (Math.Abs(delta.x) > axisIdling)
                deltaAngles.x = delta.x * RotateSense;
            if (Math.Abs(delta.y) > axisIdling)
                deltaAngles.y = delta.y * RotateSense;

            AngleX = m_Angles.x + deltaAngles.x;
            AngleY = m_Angles.y + deltaAngles.y;
        }
    }
